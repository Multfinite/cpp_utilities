...
int main() {
  ...
  auto construct_error = make_shared<string>();
  ...
  {
    Process process("ls an_incorrect_path", "", nullptr, [construct_error](const char *bytes, size_t n) {
      *construct_error += string(bytes, n);
    });
    assert(process.get_exit_status() > 0);
    assert(!construct_error->empty());
    construct_error->clear();
  }

  {
    Process process(
        ...
        [construct_error](const char *bytes, size_t n) {
          *construct_error += string(bytes, n);
        });
    assert(process.get_exit_status() > 0);
    assert(output->substr(0, 4) == "Test");
...

  {
    ...
    Process process(
        "ls an_incorrect_path", "", nullptr, [construct_error](const char *bytes, size_t n) {
          *construct_error += string(bytes, n);
        },
        false, config);
    assert(process.get_exit_status() > 0);
